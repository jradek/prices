...
/**
 * Generate two tabs
 * - Tab1: table with stores and the price per package
 * - Tab2: table with stores and the price per serving
 */
function get_tabs(
  itemId,
  itemUnit,
...
<div class="row">
  <div class="col s12">
    <ul class="tabs">
      <li class="tab col s6">
        <a class="active" href="#${id_left}">Per ${packageSize}${itemUnit} package</a>
      </li>
      <li class="tab col s6">
        <a class="active" href="#${id_right}">Per ${servingSize}${itemUnit} serving</a>
      </li>
    </ul>
  </div>
...
function processRow(data) {
  ...
  const perPackageHtml = formatStoresPerPackage(data);

  const style = "width: 80px; float: left";

  // tabs or no tabs
  const contentHtml =
    usual_package_size && usual_package_size != serving_size
      ? get_tabs(
          itemId,
          unit,
          serving_size,
...
function formatStoresPerServing(itemId) {
  ...
  const numServings = 1.0;
  const rows = perStoreData.map((store) => {
    return formatStoreRow(store, numServings, minPrice, maxPrice);
  });
...
function formatStoresPerPackage(item) {
  const itemId = item[0];
  const servingSize = item[2];
  const packageSize = item[5];
  const numServings = packageSize / servingSize;
...
/**
 * If 'numServings = 1', then this is the serving size,
 * otherwise it would be a package
 */
function formatStoreRow(data, numServings, overallMinPrice, overallMaxPrice) {
  ...
  const min_price_per_serving = data[2];
  const min_date = data[3] ? data[3] : "";
  const avg_price_per_serving = data[4];
  const max_price_per_serving = data[5];
  ...
  var min_price_per_package_s = "";
  if (min_price_per_serving) {
    const val = min_price_per_serving * numServings;
    min_price_per_package_s = `${val.toFixed(2)}&euro;`;
  }

  var avg_price_per_package_s = "";
  if (avg_price_per_serving) {
    const val = avg_price_per_serving * numServings;
    avg_price_per_package_s = `${val.toFixed(2)}&euro;`;
  }

  var max_price_per_package_s = "";
  if (max_price_per_serving) {
    const val = max_price_per_serving * numServings;
    max_price_per_package_s = `${val.toFixed(2)}&euro;`;
  }

  var regular_price_per_package_s = "";
  if (regular_price_per_serving) {
    const val = regular_price_per_serving * numServings;
    regular_price_per_package_s = `${val.toFixed(2)}&euro;`;
  }
...
<tr class="${rowStyle}">
  ...
  <td><p>${min_price_per_package_s}</p><p style="font-size: x-small">${min_date}</p></td>
  <td><p>${avg_price_per_package_s}</p><p style="font-size: x-small">&nbsp;</p></td>
  <td><p>${max_price_per_package_s}</p><p style="font-size: x-small">${max_date}</p></td>
  <td>${num_measures}</td>
  <td><p>${regular_price_per_package_s}</p><p style="font-size: x-small">${regular_date}</p></td>
</tr>
  `;
}
